...
public sealed class MenuService : IMenuService
{
    ...
    public async Task<MenuDto> GetMenuAsync(CancellationToken cancellationToken)
    {
        ...
        var itemDtos = menu.Items.Select(ClientContentMappings.MapMenuItem).ToArray();
        ...
        var sets = menu.Sets.Select(set => new MenuSetDto(set.Id, set.SetName, set.SetDescription, set.SetPrice,
            set.ImageUrl, menu.SetItems.Where(item => item.SetId == set.Id).Select(ClientContentMappings.MapMenuSetItem).ToArray())).ToArray();
