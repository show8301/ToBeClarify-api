...
- `/api/admin/orders/{id}/confirm-nominee` can be used by the nominated staff member linked to the signed-in account, or by a manager/developer as a recorded override. A manager/developer override confirms all remaining nominees in the order atomically; every nominee must otherwise confirm before the order is established.
