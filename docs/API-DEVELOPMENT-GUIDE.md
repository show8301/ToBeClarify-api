...
目前執行環境的資料庫帳號只有 `SELECT`、`INSERT`、`UPDATE` 權限，沒有刪除資料、刪除資料表或其他破壞性 DDL 權限。任何新功能、背景工作與 migration 都不得使用 `DELETE`、`DROP`、`TRUNCATE` 或等效刪除操作；請改用狀態更新、停用或取消欄位保留歷史資料。既有刪除端點與 SQL 是尚待改造的遺留程式，不得從 UI、API client 或排程呼叫。完整規範請見 [資料庫帳號權限限制](DATABASE-PERMISSIONS.md)。
...
> 下表中的 `DELETE` 端點是尚待軟刪除改造的遺留介面。受限資料庫帳號沒有刪除權限，禁止從後台 UI、API client 或排程呼叫；新增功能請使用 `PUT` 狀態更新或停用欄位。
