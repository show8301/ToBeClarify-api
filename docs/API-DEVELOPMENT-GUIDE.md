...
直接 SQL 維護時使用的資料庫帳號只有 `SELECT`、`INSERT`、`UPDATE` 權限，沒有刪除資料、刪除資料表或其他破壞性 DDL 權限。這不是 API 連線帳號的功能限制：API 可依部署連線帳號、角色授權與業務規則執行刪除流程。migration 與手動 SQL 仍不得使用受限帳號執行 `DELETE`、`DROP`、`TRUNCATE` 或等效破壞性操作。完整規範請見 [資料庫帳號權限限制](DATABASE-PERMISSIONS.md)。
...
> 下表中的 `DELETE` 是 API 業務端點，不等同於直接 SQL 維護。呼叫前須確認 API 部署連線帳號具備相應權限，並遵守角色授權、業務狀態與稽核規則；不要用提供給直接 SQL 維護的受限帳號執行同一段 SQL。
